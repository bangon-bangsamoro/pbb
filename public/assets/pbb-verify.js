...
(function (window, document) {
  ...
  function render(data) {
    ...
    /* Tell pbb-card-claim.js that a lookup landed. Only the number and the
       status travel — the masked name, chapter and dates stay in this
       function. The claim flow re-derives everything it needs from the
       server after the member proves who they are. */
    document.dispatchEvent(new CustomEvent('pbb:verified', {
      detail: { status: data.status, memberNo: data.memberNo }
    }));
  ...
  function clearResult() {
    ...
    resultEl.innerHTML = '';
    document.dispatchEvent(new CustomEvent('pbb:verified', { detail: { status: null } }));
  }
